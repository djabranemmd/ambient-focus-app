  ...
  const cards = [
    {
      icon: Timer,
      title: "Today's Focus",
      value: `${Math.floor(todayMinutes / 60)}h ${Math.floor(todayMinutes % 60)}m`,
    },
    {
      icon: Target,
      title: "Daily Goal",
    ...
    },
    {
      icon: Flame,
      title: "Streak",
      value: `${new Set(sessions.map((session) => new Date(session.date).toDateString())).size} Days`,
    },
  ];
    ...
    <Card>
      <div className="flex flex-col gap-6">
        <div>
          ...
          <p className="theme-text-muted mt-3 max-w-xl">
            Stay consistent, track your sessions, and create a better
          ...
          {cards.map((item) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-sm theme-text-muted mt-3">{item.title}</p>
