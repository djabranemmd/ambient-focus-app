...
function DailyGoal() {
  ...
  const progress = Math.min(
    (totalMinutesToday / DAILY_GOAL_MINUTES) * 100,
    100
  );

  return (
    <Card>
      <h3 className="text-xl font-semibold theme-text">Daily Goal</h3>
      <p className="theme-text-muted mt-2">Goal: 4 Hours</p>
      ...
      <p className="mt-4 text-sm theme-text-muted">
        {Math.floor(totalMinutesToday / 60)}h{" "}
        {Math.floor(totalMinutesToday % 60)}m / 4h
      </p>
    </Card>
  );
}
